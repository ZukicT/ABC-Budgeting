...
class DataExportService: ObservableObject {
    ...
    private func generateFileName(for type: ExportDataType) -> String {
        ...
        switch type {
        case .transactions:
            return "MoneyManager_Transactions_\(dateString).csv"
        case .budgets:
            return "MoneyManager_Budgets_\(dateString).csv"
        case .loans:
            return "MoneyManager_Loans_\(dateString).csv"
        case .all:
            return "MoneyManager_AllData_\(dateString).csv"
        }
    }
